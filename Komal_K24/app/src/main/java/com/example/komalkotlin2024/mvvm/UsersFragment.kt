...
import com.example.komalkotlin2024.databinding.FragmentUsersBinding
...
class UsersFragment : androidx.fragment.app.Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel = ViewModelProvider(this)[UserApiViewModel::class.java]
        viewModel.getUsers()
     ...
     private fun observeUsersChange() {
         CoroutineScope(Dispatchers.Main).launch{
             viewModel.users.collect {
                 binding.root.layoutManager = LinearLayoutManager(activity)
                 binding.root.adapter = UsersListAdapter(ArrayList(it))
             }
